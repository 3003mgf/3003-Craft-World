    ...
    <article className="fixed top-0 left-0 w-full h-screen max-h-screen bg-black/40 flex items-center justify-center z-[1000]" onClick={()=> setOpenCreate(false)}>
      ...
      <div className="bg-[url('/images/CubicPixel.jpeg')] p-3" onClick={(e)=> e.stopPropagation()}>
        <div className="bg-white border-[4px] border-white h-fit w-[400px] lg:w-[500px] p-4 flex flex-col justify-end">
          <h1 className="font-bold text-2xl">{"Hey Crafter! What's up?"}</h1>
          <p className="text-gray-400 text-xs tracking-wider mb-5 mt-2">{"Are you trying to create an empire? Great! Just tell us the name of your World and we'll create it for you! Don't worry, we'll automaticaly save your current progress."}</p>
          <div className="border-[4px] border-[#a5a592] flex flex-col">
            <input ref={inputRef} type="text" placeholder="Name your world..." className="px-3 py-3 pt-6 fonty font-light text-sm tracking-wide border-2 border-[#f5f5f5] outline-none bg-[#f7f7f7]"/>
            <button 
              className="bg-[#BFBFA9] text-white px-3 py-3 border-none text-xs font-bold hover:opacity-80 outline-none"
