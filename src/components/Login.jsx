...
const Login = () => {
  ...
  return ( 
    <AnimatePresence>
    <div className="bg-slate-400 h-screen max-h-screen w-full relative bg-[url('https://nerdy-my-ecommerce.s3.us-east-2.amazonaws.com/FrontAssets/desktop-1920x1080.jpg')] bg-cover bg-center bg-no-repeat">
      ...
      <motion.div className={`w-full flex items-center justify-center absolute ${(openLogin && !userLogged) ? "-top-[100vh]" : "top-10"} transition-all duration-400 z-50`} {...slideAnimation("down")}>
        <img src="/images/3003-World.png" alt="3003 World" className="w-[28rem] sm:w-auto cursor-pointer" onClick={()=> signOut(auth)}/>
      </motion.div>
      ...
      <div className={`flex items-center justify-center w-full space-x-20 px-2 sm:px-0 absolute left-1/2 -translate-x-1/2 transition-all duration-400 ${(openLogin && !userLogged) ? "-bottom-[100vh]" : "bottom-20"}`}>
        <motion.div className="flex items-center flex-col space-y-2 justify-center" {...slideAnimation("up")}>
          {userLogged ? (
            ...
            <>
              ...
              <img src="/images/Login.png" alt="myWorldText" className="w-[4.2rem] md:w-20" />
            </>
          )}
        </motion.div>
        <motion.div className="flex items-center flex-col space-y-2 justify-center" {...slideAnimation("up")}>
        ...
        <motion.div className="flex items-center flex-col space-y-2 justify-center" {...slideAnimation("up")}>
          <img src="/images/myWorld.png" alt="myWorld" className="w-[5.5rem] h-[5.5rem] object-contain sm:w-36 sm:h-36 md:w-40 md:h-40 hover:scale-105 transition-transform duration-300 cursor-pointer" onClick={()=> navigate("/3003world")}/>
          <img src="/images/World.png" alt="myWorldText" className="w-20 md:w-24" />
        </motion.div>
      </div>
    </div>
